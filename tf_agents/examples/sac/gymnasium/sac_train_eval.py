...
r"""Example of a SAC agent training using actor/learner in a gymnasium environment.

To run SAC on LunarLander:

```bash
tensorboard --logdir $HOME/tmp/sac_lunar_lander --port 2223 &
python tf_agents/examples/sac/gymnasium/sac_train_eval.py --root_dir=$HOME/tmp/sac_lunar_lander
```
...
flags.DEFINE_integer(
    'num_iterations', 1000000, 'Total number train/eval iterations to perform.'
)
flags.DEFINE_integer(
    'eval_interval',
    1000,
...
def train_eval(
    ...
    replay_buffer_save_interval=10000,
    eval_interval=1000,
    eval_episodes=10,
    training_goal=250.0,
  ...
  collect_env = suite_gymnasium.load(env_name, max_episode_steps=400, gym_kwargs={"continuous": True})
  eval_env = suite_gymnasium.load(env_name, max_episode_steps=400, gym_kwargs={"continuous": True}, render_kwargs={"render_mode": "rgb_array"})
  ...
  for _ in range(num_iterations):
    ...
    if eval_interval and agent_learner.train_step_numpy % eval_interval == 0:
      ...
      if training_goal and average_return > training_goal:
          break
